...
function findLargestHorizontalMaterialComponent(root, materialName) {
  const components = []
    ...
    for (const group of groups) {
      ...
      for (let offset = group.start; offset + 2 < end; offset += 3) {
        ...
        for (const vertex of vertices) {
          const connected = trianglesByVertex.get(vertex)
          if (connected) connected.push(triangleIndex)
          else trianglesByVertex.set(vertex, [triangleIndex])
        }
      }
      ...
      for (let start = 0; start < triangles.length; start++) {
        ...
        while (pending.length > 0) {
          ...
          const vertices = triangles[triangleIndex]
          for (const vertex of vertices) {
            point.fromBufferAttribute(position, vertex).applyMatrix4(obj.matrixWorld)
            box.expandByPoint(point)
            for (const connected of trianglesByVertex.get(vertex)) {
  ...
  components.sort((first, second) => second.horizontalArea - first.horizontalArea)
...
const finishLineMarker = findLargestHorizontalMaterialComponent(model, 'sha_gridlines_a')
if (!finishLineMarker) throw new Error('checkerboard finish line not found in optimized Shanghai model')
const finishMarkerCenter = finishLineMarker.center
...
const file = `// Generated by scripts/bake-racing-guide.mjs from shanghai_meshopt.glb.\n` +
  ...
  `export const SHANGHAI_FINISH_LINE_CENTER = ${JSON.stringify(
    finishMarkerCenter.toArray().map((value) => Number(value.toFixed(4))),
  )} as const\n` +
...
console.log({
  ...
  finishLine: {
    marker: 'sha_gridlines_a checkerboard',
    markerCenter: finishMarkerCenter.toArray().map((value) => Number(value.toFixed(2))),
    markerSize: finishLineMarker.size.toArray().map((value) => Number(value.toFixed(2))),
